# this python script is cited from https://github.com/wuhuikai/FaceSwap
# (partially changed)
#! /usr/bin/env python
import cv2
import numpy as np
...
def correct_colours(im1, im2, landmarks1):
    ...
    LEFT_EYE_POINTS = list(range(3, 6))
    RIGHT_EYE_POINTS = list(range(6, 9))
...
def alpha_feathering(src_img, dest_img, img_mask, blur_radius=15):
    ...
    for i in range(3):
        result_img[..., i] = src_img[..., i] * \
            mask + dest_img[..., i] * (1-mask)
...
def face_swap(src_face, dst_face, src_points, dst_points, dst_shape,   end=48):
    h, w = dst_face.shape[:2]
    # 3d warp
    warped_src_face = warp_image_3d(
        src_face, src_points[:end], dst_points[:end], (h, w))
    # Mask for blending
    ...
    unwarped_src_face = warp_image_3d(
        warped_src_face, dst_points[:end], src_points[:end], src_face.shape[:2])
    ...
    cv2.imwrite('src.png', warped_src_face)
    # Poisson Blending
